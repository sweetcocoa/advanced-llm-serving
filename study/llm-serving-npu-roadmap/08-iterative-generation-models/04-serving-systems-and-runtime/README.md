---
title: "반복 생성 모델의 Serving Systems and Runtime"
module: "반복 생성 모델"
chapter: "Serving Systems and Runtime"
format: "dialogue-lecture"
dialogue_tone: "tutoring"
estimated_reading_time: "55-75 min"
prerequisites: ["Diffusion LLM의 계보와 생성 원리", "Looped Transformer와 Recurrent Depth", "Scheduling and Admission Control"]
visuals:
  formulas: 3
  mermaid: 3
  external_images: 1
...
# 반복 생성 모델의 Serving Systems and Runtime

## 수업 개요
2026년 8월 12일 현재 diffusion LLM과 looped language model은 더 이상 모델 코드 한 파일로만 실행되는 연구 대상은 아닙니다. DiffusionGemma는 vLLM의 Model Runner V2와 `ModelState`를 통해 native batched serving 경로에 들어왔고, dInfer는 여러 dLLM을 위한 공개 inference framework를 제공합니다. 반면 Fast-dLLM, dKV-Cache, Sparse-dLLM, Sangam, Archer, continuous depth batching(CDB)은 코드 공개와 end-to-end 구현 수준이 서로 다르며, 모두 범용 production engine의 표준 기능이라고 부를 수는 없습니다. [S1][S2][S3][S8][S11][S13][S14][S15][S16]
...
| 분류 | 2026-08-12 상태 | 포함 항목 | 이 강의에서 쓰는 의미 |
| --- | --- | --- | --- |
| 릴리스된 엔진 지원 | vLLM mainline 문서·recipe에 native path가 존재 | DiffusionGemma via MRV2/`ModelState` [S1][S2][S3][S4] | 범용 serving engine 안에서 batch, scheduler, API 경로를 재사용하는 지원 |
...
### 장면 3. Ouro의 vLLM 지원은 모델을 돌리지만 adaptive exit를 보존하지 않는다
...
### 장면 4. Fast-dLLM은 cache와 parallel decoding을 결합한다
...
### 장면 5. dKV-Cache와 Sparse-dLLM은 같은 cache 문제를 다른 방향으로 푼다
...
### 장면 6. dInfer는 알고리즘 묶음이 아니라 조합 가능한 inference framework를 지향한다
...
### 장면 7. cached dLLM은 prefill과 decode가 한 번씩이 아니라 반복된다
...
### 장면 8. Archer는 rollback 가능한 부분을 cache 밖에 둔다
...
### 장면 9. Continuous depth batching은 loop iteration을 scheduling 단위로 바꾼다
...
**학습자:** 논문 결과는 adaptive exit의 theoretical saving을 실제로 회수했습니까?
...
### 장면 10. Diffusion step과 loop depth는 결국 2차원 scheduling 문제를 만든다
...
### 장면 11. 현재 도입 판단은 feature matrix보다 evidence ladder로 한다
...
### 장면 12. benchmark 표보다 먼저 실험 계약서를 쓴다
...
## 핵심 정리
- 2026-08-12 기준 DiffusionGemma는 MRV2 `ModelState`와 diffusion sampler를 이용하는 vLLM native 지원 사례다. [S1][S2][S3][S4]
