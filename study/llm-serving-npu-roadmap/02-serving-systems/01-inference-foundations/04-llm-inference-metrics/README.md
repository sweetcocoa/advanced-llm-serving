...
**학습자:** 같은 total latency 안에서도 사용자가 기다리는 침묵 구간과 스트리밍 리듬을 따로 봐야 하나요?
...
**학습자:** `TTFT`가 나쁘다는 제보를 받으면 queue, prefill, first decode 중 무엇부터 분리해야 합니까?
...
**학습자:** `TPOT`이 흔들릴 때 peak FLOPs만 보면 놓치는 신호가 무엇인가요?
...
**학습자:** batch를 키울수록 좋은 서비스와, 오히려 대기 체감이 나빠지는 서비스를 어떤 지표로 구분합니까?
