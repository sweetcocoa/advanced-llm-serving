...
prerequisites: ["Transformer Inference", "Memory, Bandwidth, Latency"]
visuals:
  formulas: 2
  mermaid: 2
  external_images: 2
...
**학습자:** 정책 검색 챗봇처럼 입력은 길고 출력은 짧다면, 평균 latency보다 `TTFT` 분해가 먼저겠네요?
...
**학습자:** 첫 토큰은 빠른데 뒤가 늘어진다면, 모델 크기보다 토큰당 지연과 batching 상태를 먼저 의심해야 하나요?
...
**학습자:** 한 풀에서 긴 입력 요청과 긴 출력 요청이 서로 밀어내면, 그때 분리형 serving을 검토하는 건가요?
...
**학습자:** 장애 티켓에도 "모델이 느림" 대신 "TTFT가 느림" 또는 "decode step이 느림"으로 적어야겠네요.
