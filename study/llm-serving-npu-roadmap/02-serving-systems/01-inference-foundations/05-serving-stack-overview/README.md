...
prerequisites: ["LLM Inference Metrics"]
visuals:
  formulas: 2
  mermaid: 2
  external_images: 2
...
**학습자:** API 모양이 같아도 SLO, fallback, 배포 책임이 다르면 사실상 다른 운영 계약이 되는 거네요.
...
**학습자:** 첫 토큰이 느릴 때 kernel profiler부터 켜면 어떤 상위 계층 병목을 놓치기 쉽습니까?
...
**학습자:** kernel 최적화로 내려가기 전에 API, scheduler, cache, backend 중 어떤 증거가 먼저 있어야 하나요?
