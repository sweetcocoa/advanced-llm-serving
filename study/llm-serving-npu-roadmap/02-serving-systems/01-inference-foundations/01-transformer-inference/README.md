...
**학습자:** 같은 모델을 쓰는데도 서비스별로 먼저 봐야 할 지표가 달라지나요?
...
**학습자:** attention만 보면 비용이 과소평가된다는 말인가요? projection과 FFN도 같은 수준으로 비용표에 넣어야 합니까?
...
**학습자:** 그러면 첫 토큰 지연과 후반 토큰 지연을 같은 그래프에 올려도, 원인 후보는 다르게 잡아야겠네요.
...
**학습자:** 긴 문맥이 실제 품질을 올리는 서비스와, 단지 비용만 늘리는 서비스를 어떻게 가르나요?
...
**학습자:** 병목을 줄이는 일보다, 그 병목을 감수할 이유가 있는지 먼저 묻는 경우도 있겠네요.
...
**학습자:** [S4]는 직접 알고리즘 근거라기보다 최신성 근거처럼 보입니다. 이 챕터에서 너무 강하게 읽히지는 않나요?
