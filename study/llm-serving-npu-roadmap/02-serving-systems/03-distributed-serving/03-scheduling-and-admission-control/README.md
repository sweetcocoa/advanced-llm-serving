...
prerequisites: ["Continuous Batching", "LLM Inference Metrics"]
visuals:
  formulas: 2
  mermaid: 2
  local_images: 2
...
## 참고 이미지
아래 두 도식은 이 챕터의 운영 실패 모양을 직접 비교하기 위해 작성했다. 일반적인 하드웨어 그림 대신 queue 정책과 재시도 파형처럼 scheduler가 실제로 제어하는 대상을 보여 준다.

![Queue fairness snapshot](./assets/img-01.svg)
- 캡션: 요청 비용만 우선할 때 긴 요청이 굶는 경로와 aging·tenant deficit으로 진행률을 보정하는 지점을 비교한다.

![Retry burst amplification timeline](./assets/img-02.svg)
- 캡션: 모든 client에 같은 `retry-after`를 줄 때 재시도 시점이 다시 겹쳐 queue oscillation이 반복되는 과정을 나타낸다.
