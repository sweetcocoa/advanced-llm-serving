...
## 객관식 8
2026년 8월 14일 공식 자료에 근거한 엔진 지원 설명으로 가장 정확한 것은 무엇인가?

1. TensorRT-LLM의 image/video diffusion beta는 text dLLM native 지원과 같은 기능이다.
2. SGLang은 LLaDA2.x를 지원하지만 roadmap의 모든 최적화가 완료됐다고 단정할 수는 없다.
3. LMDeploy에 SDAR가 있으므로 모든 masked diffusion checkpoint가 자동 지원된다.
4. vLLM의 `DiffusionConfig`만 있으면 모델별 `ModelState`가 필요 없다.
...
- 객관식 8 정답: 2. SGLang의 architecture tracker와 roadmap은 LLaDA2.x 실행 경로와 완료·진행 중인 최적화를 함께 보여 준다. LMDeploy의 SDAR 지원은 모델 범위가 있고, TensorRT-LLM의 diffusion beta는 공식 표에서 image/video generation을 뜻한다. [S20][S21][S22][S23]
