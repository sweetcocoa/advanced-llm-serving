---
title: "Long-Context Serving and KV Memory"
module: "서빙 시스템 설계"
chapter: "Long-Context Serving and KV Memory"
format: "dialogue-lecture"
dialogue_tone: "tutoring"
estimated_reading_time: "35-50 min"
prerequisites: ["KV Cache and PagedAttention", "LLM Inference Metrics"]
visuals:
  formulas: 2
  mermaid: 2
  local_images: 2
...
# Long-Context Serving and KV Memory
...
## 참고 이미지
![KV block reuse and host offload flow](./assets/img-01.svg)

- [I1] 제목: KV block reuse and host offload flow
- 출처: TensorRT-LLM KV cache 문서를 바탕으로 재구성한 로컬 SVG, `img-01.svg`
- 이 챕터에서 보는 포인트: 공통 prefix block 재사용과 host offload가 서로 다른 비용을 줄인다는 점을 한 경로에서 비교한다 [S3][S4][S6].

![Long-context serving diagnostic map](./assets/img-02.svg)

- [I2] 제목: Long-context serving diagnostic map
- 출처: vLLM과 TensorRT-LLM 문서를 바탕으로 재구성한 로컬 SVG, `img-02.svg`
- 이 챕터에서 보는 포인트: prefill compute, live KV residence, KV handoff를 하나의 메모리 문제로 뭉개지 않고 진단 순서로 분리한다 [S1][S2][S4][S5][S6].
