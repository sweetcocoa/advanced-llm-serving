...
prerequisites: ["Serving Stack Overview", "Prefill vs Decode"]
visuals:
  formulas: 2
  mermaid: 2
  external_images: 2
...
**학습자:** throughput을 올리려고 동시성을 늘렸는데 KV cache가 먼저 찬다면, 병목은 scheduler가 아니라 memory budget으로 봐야 하나요?
...
**학습자:** PagedAttention을 압축 기술처럼 설명하면 어디가 틀립니까?
...
**학습자:** 논리적으로는 연속된 sequence인데 물리 block은 흩어질 수 있다면, 어떤 메타데이터가 성능에 중요해집니까?
...
**학습자:** "메모리가 부족하다"는 결론 전에 cache block 크기, fragmentation, 요청 길이 분포를 같이 봐야겠네요.
