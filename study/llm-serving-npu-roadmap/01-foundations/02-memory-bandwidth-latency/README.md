...
**학습자:** 그럼 모델을 줄였다는 사실만으로는 부족하고, 실제로 줄어든 항이 어느 쪽인지 확인해야겠네요.
...
**학습자:** Roofline을 성능 예측기라기보다, 더 파야 할 축을 고르는 중단 조건으로 써도 되나요?
...
**학습자:** prefill/decode를 이미 배웠어도, 여기서는 구조 복습보다 병목 분류표로 다시 쓰는 거네요.
...
**학습자:** 이 그림에서는 새 토큰 계산보다 과거 상태를 다시 읽는 경로가 더 크게 보입니다. 실제 로그에서도 그 차이를 분리해서 볼 수 있나요?
...
**학습자:** 같은 모델이어도 문맥 길이와 출력 패턴이 다르면, 같은 최적화 계획을 복사하면 안 되겠네요.
...
**학습자:** 최신성 근거는 `항상 bandwidth가 병목이다`가 아니라, runtime이 바뀌어도 memory movement를 계속 계측해야 한다는 쪽으로 읽어야겠네요.
