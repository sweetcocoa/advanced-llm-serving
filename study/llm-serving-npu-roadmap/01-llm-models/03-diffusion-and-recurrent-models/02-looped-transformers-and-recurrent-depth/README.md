...
prerequisites: ["Transformer와 Autoregressive Language Model", "Diffusion LLM의 계보와 생성 원리"]
visuals:
  formulas: 6
  mermaid: 3
  external_images: 1
...
이 계보는 2016년 Adaptive Computation Time(ACT), 2018년 Universal Transformer에서 시작해 fixed-point를 직접 푸는 Deep Equilibrium Model(DEQ)과 이웃한다. 2023년 looped Transformer가 iterative learning algorithm을 학습하는 구조로 다시 부상했고, length generalization, Huginn, Mixture-of-Recursions, Ouro를 거치며 LLM 규모의 latent computation과 adaptive depth로 확장됐다 [S1][S2][S4][S5][S6][S7][S9][S10]. 2026년에는 LoopFormer, `Loop, Think, & Generalize`, LOTUS, Source-Centered State Evolution(SCSE)이 반복 횟수 외삽, 과잉 반복, latent supervision, 상태 동역학을 더 정교하게 다뤘다 [S11][S12][S13][S14]. DeepLoop는 공유 block을 여러 번 방문할 때 residual scaling 자체가 달라져야 한다고 분석했고, Looped Mamba/Hybrid는 recurrent depth가 Transformer에만 묶인 개념이 아님을 보였다 [S15][S16].
...
- tied-depth parameter visit를 반영한 residual scaling과 일반 dense-depth scaling의 차이를 설명할 수 있다.
- adaptive exit-state selection과 실제 early exit를 구분할 수 있다.
...
### Part 10. 반복 안정성과 backbone 확장은 별도 문제다
**교수자:** 지금까지는 주로 몇 번 반복할지와 언제 멈출지를 봤습니다. 그런데 shared block을 반복하면 학습 안정성도 달라집니다. 일반적인 깊은 Transformer에서는 각 residual branch가 서로 다른 파라미터를 가집니다. looped model에서는 같은 branch가 여러 depth 위치에서 재방문되고, 그 방문들의 gradient가 하나의 파라미터 update에 모입니다.

**학습자:** unrolled depth가 24라면 24-layer Transformer의 residual scaling을 그대로 쓰면 되지 않습니까?

**교수자:** nominal depth만 같다고 update 구조까지 같지는 않습니다. DeepLoop는 반복 방문의 정렬 정도를 visit-alignment coefficient로 분석합니다. 방문 효과가 서로 decorrelate되면 기존 DeepNorm의 scaling과 가까워지지만, 보수적인 aligned regime에서는 loop 수가 늘 때 더 강한 scaling이 필요하다고 주장합니다. 제안된 Post-LN 설정은 unrolled depth $N$에 대해 $\alpha=(2N)^{1/2}$, $\beta=(8N)^{-1/2}$를 사용합니다 [S15]. 핵심은 이 상수를 모든 looped architecture의 정답으로 외우는 것이 아니라, **physical layer 수가 아니라 공유 파라미터가 몇 번 방문되는지**도 안정성 분석에 들어가야 한다는 점입니다.

**학습자:** recurrent depth는 결국 Transformer block을 반복하는 기법입니까?

**교수자:** 그보다 넓습니다. Looped State-Space Language Models 연구는 shared Mamba block과 Mamba-Transformer hybrid block을 반복했습니다. 통제된 reasoning task에서는 parameter-matched non-looped baseline보다 나은 결과를 보고했고, language-model pretraining에서는 적은 distinct parameter로 경쟁력을 보였습니다. 다만 strict iso-FLOPs 비교에서는 더 깊은 non-looped model이 validation perplexity에서 우세한 경우도 있었습니다 [S16]. 즉 반복 깊이는 backbone과 독립적으로 적용할 수 있는 설계 축이지만, 어떤 backbone에서도 공짜 성능 향상을 보장하지는 않습니다.

**학습자:** 그 논문의 adaptive exit gate가 중간 state를 고르면 앞의 loop만 실행한 것 아닙니까?

**교수자:** 아닙니다. 해당 실험의 gate는 모든 recurrent step을 실행한 뒤 어느 step의 출력을 prediction state로 쓸지 선택합니다. 선택된 exit step은 **예측에 사용한 깊이**이지 **실제로 실행을 멈춘 깊이**가 아닙니다 [S16]. 중간 state를 고른 것만으로 뒤의 loop 계산이 사라지지 않으므로 wall-clock 절감도 생기지 않습니다. 실제 early exit가 되려면 gate 결정을 실행 도중 내리고, 이후 state update를 건너뛰며, batch에서 먼저 끝난 항목을 처리하는 runtime까지 필요합니다.

| 구분 | 질문 | 모델 결과가 말해 주는 것 | 별도로 검증할 것 |
| --- | --- | --- | --- |
| Residual stability [S15] | 같은 branch의 반복 방문이 update를 얼마나 정렬시키는가? | tied-depth에 맞는 scaling 필요성 | 다른 정규화·규모·optimizer로의 전이 |
| Backbone extension [S16] | recurrence가 attention-only 구조에 한정되는가? | Mamba와 hybrid에서도 적용 가능 | 실제 LM 품질과 hardware efficiency |
| Exit-state selection [S16] | 어느 반복 state가 예측에 좋은가? | 중간 state 선택으로 품질 개선 가능 | 실행 step 감소 여부 |
| True early exit | 뒤의 반복을 실제 생략할 수 있는가? | 위 선택 실험만으로는 입증되지 않음 | online gate, state/KV 처리, wall-clock latency |

### Part 11. SCSE와 2026년 8월의 상태 동역학
...
flowchart LR
    ...
    H --> Q["2026 LOTUS"]
    H --> P["2026 DeepLoop scaling"]
    H --> B["2026 Looped Mamba / Hybrid"]
    U --> S["2026 SCSE"]
```

**학습자:** 2026년 8월 14일 기준으로 이 계보의 가장 방어적인 결론은 무엇입니까?
...
- exit-state selection은 모든 loop를 실행한 뒤 readout state를 고를 수도 있다. 실제 early exit와 wall-clock 절감은 별도 주장이다 [S16].
- weight tying은 parameter 수뿐 아니라 residual update의 누적 방식도 바꾼다. nominal unrolled depth만으로 안정성을 판단하면 부족하다 [S15].
...
| Looped Mamba / Hybrid [S16] | shared state-space 또는 hybrid block 반복 | recurrent hidden state | Transformer 밖으로 depth recurrence 확장 | exit-state 선택만으로 실행량은 줄지 않음 |
...
- DeepLoop는 shared residual branch의 반복 방문을 반영한 scaling이 필요함을 제안하고, Looped Mamba/Hybrid는 recurrent depth를 state-space backbone으로 확장한다 [S15][S16].
- 좋은 중간 exit state를 찾는 것과 뒤의 recurrent step을 실제로 생략하는 것은 다르다. 후자만 wall-clock 계산 절감이 될 수 있다 [S16].
- SCSE는 anchor와 deviation을 분리해 반복 상태 동역학을 구조적으로 안정화하려는 2026년 7월의 최신 제안이다.
- 2026년 8월 14일 현재 recurrent depth는 새로운 LLM scaling axis로 볼 근거가 있지만, production efficiency는 runtime과 함께 측정해야 한다.
...
- DeepLoop가 physical depth 외에 parameter visit를 세는 이유를 설명할 수 있는가?
- Looped Mamba의 exit-state selection이 실제 early exit가 아닌 이유를 설명할 수 있는가?
...
| [S15] | DeepLoop: Depth Scaling for Looped Transformers | Li et al. | 2026-07-15 | [https://arxiv.org/abs/2607.13491](https://arxiv.org/abs/2607.13491) | tied-depth parameter visit를 반영한 residual scaling과 학습 안정성 |
| [S16] | Looped State-Space Language Models with Adaptive Exit-State Selection | Yu et al. | 2026-07-11 | [https://arxiv.org/abs/2607.10110](https://arxiv.org/abs/2607.10110) | Looped Mamba/Hybrid와 exit-state selection의 실행 경계 |
