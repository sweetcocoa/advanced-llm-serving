...
## 객관식 4
Looped Mamba 연구의 adaptive exit-state selection을 가장 정확히 설명한 것은 무엇인가?

1. 선택된 step 뒤의 모든 recurrent computation을 실행하지 않는다.
2. 모든 recurrent step을 실행한 뒤 prediction에 사용할 중간 state를 선택하므로, 선택 깊이만으로 wall-clock 절감을 주장할 수 없다.
3. Mamba block을 Transformer block으로 교체하는 router다.
4. sequence의 출력 token 수를 줄이는 방법이다.
...
## 짧은 서술형 4
DeepLoop가 일반 DeepNorm식 nominal depth 외에 shared residual branch의 반복 방문을 분석해야 한다고 주장하는 이유를 설명하라 [S15].
...
- 객관식 4 정답: 2. 논문의 gate는 prediction depth를 선택하지만 모든 recurrent step은 이미 실행한다. 실제 계산 절감에는 online halting과 이후 step 생략이 필요하다 [S16].
...
- 짧은 서술형 4 예시: untied deep Transformer에서는 각 residual branch가 별도 파라미터 update를 받지만, looped model에서는 같은 branch가 여러 depth에서 재사용되고 방문별 gradient가 하나의 update에 모인다. 따라서 unrolled layer 수만 같은 scaling은 반복 방문 효과를 놓칠 수 있다.
