...
estimated_reading_time: "60-80 min"
prerequisites: ["Transformer와 Autoregressive Language Model"]
visuals:
  formulas: 4
  mermaid: 3
  external_images: 1
...
이 챕터는 이미지 diffusion의 비유에서 멈추지 않는다. 연속 embedding diffusion, categorical/discrete diffusion, absorbing-mask diffusion, score-based diffusion, block diffusion을 구분하고, LLaDA와 Dream 이후 대규모 diffusion LLM이 무엇을 바꿨는지 살펴본다. 특히 2025년 후반부터 뚜렷해진 AR-to-block-diffusion 전환 계보를 독립된 흐름으로 다룬다. 마지막에는 2026년 8월 최신 연구가 드러낸 commitment order와 pretraining-generation mismatch까지 연결한다 [S15][S16][S18][S19][S20][S21].
...
- from-scratch dLLM과 AR-to-block-diffusion conversion의 학습·추론 계약을 비교할 수 있다.
...
flowchart LR
    ...
    P["pretrained AR backbone"] --> F["2025 Fast-dLLM v2"]
    P --> A["2025 SDAR"]
    P --> X["2025 LLaDA2.0 16B/100B MoE"]
    P --> G["2026 DiffusionGemma 26B-A4B"]
    K --> F["2025 Fast-dLLM v2"]
    K --> A["2025 SDAR"]
    K --> X
    K --> G
...
### Part 5. AR에서 block diffusion으로 전환하는 계보
**학습자:** Dream도 AR pretrained model에서 시작했다면, AR-to-diffusion 전환은 이미 설명한 것 아닌가요?

**교수자:** 초기화와 생성 계약의 전환을 구분해야 합니다. Dream은 AR 모델을 초기값으로 삼아 양방향 masked diffusion model로 적응시켰습니다 [S9]. 2025년 후반의 새 흐름은 여기에 `block` 경계를 명시합니다. 이미 학습된 AR의 장거리 문맥 처리와 지식을 최대한 유지하면서, block 내부만 병렬 denoising으로 바꾸려는 접근입니다. block 사이에는 여전히 왼쪽에서 오른쪽으로 진행하므로 완전 양방향 diffusion과 순수 AR의 중간 지점입니다.

Fast-dLLM v2는 pretrained AR model을 약 1B adaptation token으로 block dLLM에 맞추는 recipe를 제안했습니다. complementary attention mask로 AR objective를 유지하면서 block 안의 bidirectional context modeling을 함께 학습하고, block cache와 sub-block cache를 나눠 parallel decoding을 지원합니다 [S18]. 논문이 보고한 최대 2.5배 speedup은 해당 모델·hardware·decoding 설정의 결과이지, 전환 recipe만 적용하면 항상 얻는 상수는 아닙니다.

**학습자:** SDAR도 같은 방법입니까?

**교수자:** 목표는 비슷하지만 강조점이 다릅니다. SDAR는 AR pretraining의 compute efficiency를 출발점으로 삼고, 짧은 adaptation 뒤 block 사이를 autoregressive하게, block 안을 discrete diffusion으로 생성합니다. dense와 MoE scaling 실험을 통해 큰 모델일수록 block size와 decoding threshold 변화에 더 강인할 수 있다는 결과를 제시했습니다 [S19]. Fast-dLLM v2의 구체적인 complementary mask와 hierarchical cache를 SDAR 전체의 정의로 일반화해서는 안 됩니다.

LLaDA2.0은 이 전환을 frontier scale로 밀어 올렸습니다. AR model의 지식을 계승하면서 block size를 늘리고, full-sequence diffusion을 거쳐 다시 compact block으로 돌아오는 3단계 WSD adaptation을 사용해 16B와 100B MoE variant를 만들었습니다 [S20]. 이것은 `diffusion LM은 반드시 처음부터 다시 pretrain해야 한다`는 전제를 약화하지만, AR checkpoint만 바꾸면 공짜로 병렬 생성이 생긴다는 뜻은 아닙니다. attention contract, corruption objective, post-training, sampler를 다시 맞춰야 합니다.

**학습자:** DiffusionGemma도 이 전환 계보에 놓을 수 있습니까?

**교수자:** 넓게는 그렇습니다. DiffusionGemma는 Gemma 4의 26B-A4B MoE 기반을 사용하면서, prompt는 autoregressive encoder가 읽어 KV cache로 만들고 256-token canvas는 bidirectional decoder가 반복 denoising합니다. 완성된 canvas를 context에 붙인 뒤 다음 canvas로 넘어가는 multi-canvas block-autoregressive 구조입니다 [S21]. 따라서 `AR 계열 backbone 자산의 활용`, `block 내부 diffusion`, `block 사이 AR 진행`이라는 설계 축을 공유합니다. 다만 Fast-dLLM v2나 SDAR와 동일한 변환·학습 recipe라고 쓰면 안 됩니다.

| 전환 계열 | AR에서 가져오는 것 | block 안 생성 | 규모·특징 | 읽을 때 주의할 점 |
| --- | --- | --- | --- | --- |
| Fast-dLLM v2 [S18] | pretrained AR model과 AR objective | block diffusion + complementary mask | 약 1B-token adaptation, hierarchical cache | 보고 speedup은 실험 조건 의존 |
| SDAR [S19] | compute-efficient AR pretraining | discrete diffusion | dense/MoE scaling, 짧은 adaptation | 별도 paradigm의 공통 원리를 cache 구현과 혼동하지 않기 |
| LLaDA2.0 [S20] | AR knowledge와 MoE backbone | block/full-sequence/block의 단계적 적응 | 16B, 100B MoE | parameter scale과 active compute를 분리해 읽기 |
| DiffusionGemma [S21] | Gemma 4 기반 encoder와 cached prompt context | 256-token canvas의 uniform-state diffusion | 26B-A4B, encoder-decoder, multi-canvas | model-card 수치와 독립 비교 benchmark를 구분하기 |

이 계보의 의미는 AR과 diffusion이 서로 배타적인 진영이 아니라는 데 있습니다. 학습은 AR의 데이터·compute 효율을 활용하고, 생성은 block 내부 병렬 refinement를 사용하며, 긴 sequence는 block 사이 AR factorization으로 처리할 수 있습니다. 대신 block size가 커질수록 한 pass의 병렬성은 커지지만 token dependency와 재계산 부담도 커집니다. `어디까지 AR이고 어디부터 diffusion인가`가 새로운 모델 설계 변수가 된 것입니다.

### Part 6. Parallel decoding의 이득과 함정
...
### Part 7. 순서 자유도는 장점인 동시에 sampler 책임이다
...
### Part 8. 2026년 8월, 규모와 interface가 동시에 문제다
...
- AR-to-block-diffusion conversion은 AR 지식을 보존하려는 계보이지, 학습 없이 attention과 sampler가 자동으로 바뀐다는 뜻이 아니다 [S18][S19][S20].
...
- 계보는 continuous embedding과 discrete token 두 갈래에서 출발해 SEDD, MDLM, LLaDA, Dream, block diffusion으로 확장됐고, 2025년 후반부터 AR 모델을 block diffusion으로 전환하는 흐름이 큰 축이 됐다.
- Fast-dLLM v2, SDAR, LLaDA2.0, DiffusionGemma는 AR의 학습 자산과 block 내부 병렬 denoising을 결합하지만 학습 recipe와 inference architecture는 서로 다르다 [S18][S19][S20][S21].
...
- Fast-dLLM v2, SDAR, LLaDA2.0, DiffusionGemma가 AR과 diffusion을 결합하는 방식을 비교할 수 있는가?
...
| [S18] | Fast-dLLM v2: Efficient Block-Diffusion LLM | Wu et al. | 2025-09-30 | [https://arxiv.org/abs/2509.26328](https://arxiv.org/abs/2509.26328) | 약 1B token으로 AR model을 block dLLM으로 적응하는 recipe와 hierarchical cache |
| [S19] | SDAR: A Synergistic Diffusion-AutoRegression Paradigm for Scalable Sequence Generation | Cheng et al. | 2025-10-07 | [https://arxiv.org/abs/2510.06303](https://arxiv.org/abs/2510.06303) | AR pretraining과 block 내부 discrete diffusion을 결합한 전환 계보 |
| [S20] | LLaDA2.0: Scaling Up Diffusion Language Models to 100B | Bie et al. | 2025-12-10 | [https://arxiv.org/abs/2512.15745](https://arxiv.org/abs/2512.15745) | 3단계 AR-to-dLLM adaptation과 16B/100B MoE scaling |
| [S21] | DiffusionGemma Model Card | Google DeepMind | 2026-06-10 | [https://ai.google.dev/gemma/docs/diffusiongemma/model_card](https://ai.google.dev/gemma/docs/diffusiongemma/model_card) | Gemma 4 기반 encoder-decoder와 256-token multi-canvas diffusion의 공식 사양 |
